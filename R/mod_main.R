...
mod_main_ui <- function(id){
  ...
  tagList(
    ...
    splitLayout(
      wellPanel(
        wellPanel(
          textInput(inputId = ns("equation"), "Equation:", "Vmax * C / (Km + C)"),
          actionButton(ns("updatevars"), "Update variables")
        ),
        uiOutput(ns("variables"))
      ),
      wellPanel(
        plotOutput(ns("plot")),
        actionButton(ns("addSnapshot"), "Add Snapshot"),
        actionButton(ns("removeSnapshots"), "Remove Snapshots"),
        hr(),
        p(strong("Axis limits")),
        splitLayout(
          cellWidths = c("20%", "20%"),
          numericInput(ns("xLower"), value = 0, label = "x min", step = 0.1),
          numericInput(ns("xUpper"), value = 100, label = "x max", step = 0.1)
        ),
        splitLayout(
          cellWidths = c("20%", "20%"),
          numericInput(ns("yLower"), value = 0, label = "y min", step = 0.1),
          numericInput(ns("yUpper"), value = 100, label = "y max", step = 0.1)
        ),
        hr(),
        p(strong("Add single points to plot")),
        splitLayout(
          cellWidths = c("20%", "20%"),
          numericInput(ns("xvalue"), value = 0, label = "x", step = 0.01),
          numericInput(ns("yvalue"), value = 0, label = "y", step = 0.01)
        ),
        actionButton(ns("addPoint"), "Add Point"),
        actionButton(ns("removePoints"), "Remove All Points"),
        hr(),
        p(strong("Add data from .xlsx file")),
        fileInput(ns("chooseFile"), label = NULL, accept = ".xlsx"),
        actionButton(ns("removeLastObsData"), "Remove last added data"),
        actionButton(ns("removeObsData"), "Remove all uploaded data")
      )
    )
  )
}
      ...
      isolate({
        ...
        variable_list <- lapply(seq_along(v$varnames), function(i) {
          splitLayout(
            ...
            numericInput(session$ns(v$mins[i]), "min", 0),
            sliderInput(session$ns(v$varnames[i]), v$varnames[i], 0, 100, 1),
            numericInput(session$ns(v$maxs[i]), "max", 100),
            numericInput(session$ns(v$current[i]), "current", 1)
          )
        })
    ...
    observeEvent(input$chooseFile, {
      ...
      if (grepl("(.xlsx$)|(.xls$)", file$name)) {
        ...
        showModal(modalDialog(
          ...
          selectInput(session$ns("fileX"), "x axis", choices = names(content)),
          selectInput(session$ns("fileY"), "y axis", choices = names(content)),
          textInput(session$ns("fileLabel"), "Legend label", placeholder = "optional - legend label in plot"),
